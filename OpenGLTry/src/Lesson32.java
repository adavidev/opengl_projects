...
public class Lesson32 {
    ...
    Texture textures[];                                      // Storage For 10 Textures
    Timer timer;
...
//                mainloop((currentTime - lastTime) / timerResolution);
    ...
    private void init() throws Exception {
        game = false;
        textures = new Texture[10];                                      // Storage For 10 Textures
...
//      GL11.glEnable(GL11.GL_ALPHA_TEST);                                    // Enable Alpha Testing  (disable blending)
    ...
    private void buildFont() {                                  // Build Our Font Display List
        ...
        textures[9].bind();     // Select Our Font Texture
...
//            playSound("data/shot.wav",NULL,SND_ASYNC);                  // Play Gun Shot Sound
    ...
    void drawObject(float width, float height, int texid) {             // Draw Object Using Requested Width, Height And Texture
        textures[texid].bind();        // Select The Correct Texture
    ...
    void explosion(int num) {                                        // Draws An Animated Explosion For Object "num"
        ...
        textures[5].bind();            // Select The Explosion Texture
    ...
    private void render() {
        ...
        GL11.glPushMatrix();                                             // Push The Modelview Matrix
        textures[7].bind();            // Select The Sky Texture
        ...
        textures[6].bind();            // Select The Ground Texture
...
//        AL10.alSourcei(source.get(0), AL10.AL_BUFFER, buffer.get(0));
    ...
    private void loadTextures() {                                   // Load Bitmaps And Convert To Textures
        textures[0] = loadTexture("BlueFace.tga");         // Load The BlueFace Texture
        textures[1] = loadTexture("Bucket.tga");           // Load The Bucket Texture
        textures[2] = loadTexture("Target.tga");           // Load The Target Texture
        textures[3] = loadTexture("Coke.tga");             // Load The Coke Texture
        textures[4] = loadTexture("Vase.tga");             // Load The Vase Texture
        textures[5] = loadTexture("Explode.tga");          // Load The Explosion Texture
        textures[6] = loadTexture("Ground.tga");           // Load The Ground Texture
        textures[7] = loadTexture("Sky.tga");              // Load The Sky Texture
        textures[8] = loadTexture("Crosshair.tga");        // Load The Crosshair Texture
        textures[9] = loadTexture("Font.tga");               // Load The Crosshair Texture
    }

    private final Texture loadTexture(String path) {
		Texture texture = null;
    	
    	try
		{
			texture = TextureLoader.getTexture("tga", ResourceLoader.getResourceAsStream(path),true, GL_LINEAR);
